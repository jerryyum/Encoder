...
static void BarcodeWriteBits(barcode_t* barcode, uint16_t pattern, int width)
{
    ...
    {
        size_t byteOffset = barcode->offset / 8;
...
{
    if (barcode->error || barcode->code == newCode)
        return;
    ...
    if (barcode->code == BARCODE_CODE_NONE)
    {
        ...
        {
        case BARCODE_CODE_A:
            BarcodeAppendSymbol(barcode, 103);
            break; // START (Code A)
        case BARCODE_CODE_B:
            BarcodeAppendSymbol(barcode, 104);
            break; // START (Code B)
        case BARCODE_CODE_C:
            BarcodeAppendSymbol(barcode, 105);
            break; // START (Code C)
        default:
            barcode->error = true;
            break;
    ...
    else
    {
        ...
        {
        case BARCODE_CODE_A:
            BarcodeAppendSymbol(barcode, 101);
            break; // (switch) CODE A
        case BARCODE_CODE_B:
            BarcodeAppendSymbol(barcode, 100);
            break; // (switch) CODE B
        case BARCODE_CODE_C:
            BarcodeAppendSymbol(barcode, 99);
            break; // (switch) CODE C
        default:
            barcode->error = true;
            break;
...
static void BarcodeStop(barcode_t* barcode)
{
    ...
    if (barcode->code == BARCODE_CODE_STOP)
        return;
...
void BarcodeAppend(barcode_t* barcode, const char* text)
{
    for (const char* p = text; *p != '\0'; p++)
    {
        ...
        if (c0 >= 0 && c0 < 0x20)
            requiredCode = BARCODE_CODE_A;
        else
            requiredCode = BARCODE_CODE_B;
        // if (c0 >= '\'' && c0 <= 0x7f)
        // requiredCode = BARCODE_CODE_B;
...
BARCode BARCode::encodeText(const char* text)
{
    return BARCode(text);
}

BARCode::BARCode(const char* text)
{
    size = BARCODE_SIZE_TEXT(14, BARCODE_QUIET_NONE);
    bitmap = new uint8_t[size];
    size = en_barcode(bitmap, size, BARCODE_QUIET_NONE, text);
}
...
int BARCode::getSize() const
{
    return size;
}

bool BARCode::getModule(int x) const
{
    assert(x < size);
    bool bit = BARCODE_BIT(bitmap, x);
    return bit;
}
